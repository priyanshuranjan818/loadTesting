...
| Animation      | Framer Motion                     | Smooth layout + enter/exit animations    |
| Icons          | Lucide + Heroicons                | Consistent icon libraries                |
| Charts         | Recharts                          | Simple, composable charting library      |
| Hosting        | S3 + CloudFront                   | Free tier, global CDN, HTTPS out of box  |
| API layer      | API Gateway HTTP API (v2)         | Managed, scales to zero, free tier       |
| Orchestration  | AWS Step Functions                | Parallel Map state fans out to N workers |
| Workers        | AWS Lambda (Node.js 20.x)         | Deployed in 3 regions simultaneously    |
...
│   │   ├── startTest/
│   │   │   └── index.js          # receives config, distributes workers across regions
│   │   └── getResults/
│   │       └── index.js          # queries DynamoDB, calculates p50/p95/p99
│   └── statemachine/
│       └── definition.json       # Step Functions state machine (ItemProcessor, dynamic lambdaArn)
├── frontend/
│   ├── src/
│   │   ├── App.jsx               # main layout + polling logic + nav
│   │   ├── TestForm.jsx          # URL input, concurrency, request count
│   │   ├── ResultsDashboard.jsx  # stat cards + regional bar chart
│   │   ├── About.jsx             # About page (bento grid layout)
│   │   ├── Contact.jsx           # Contact page
│   │   ├── Docs.jsx              # In-app documentation panel
│   │   ├── index.css             # TailwindCSS v4 base + design tokens
│   │   └── main.jsx              # React entry point
│   ├── index.html
│   ├── .env.example              # copy to .env and set VITE_API_URL
│   └── vite.config.js
├── terraform/
│   └── main.tf                   # provisions ALL AWS infrastructure (3 regions)
└── CLAUDE.md                     # this file
```
...
```json
{
  "testId":        "test-1712345678901-abc123",
  "requestId":     "META",
...
- `ItemProcessor` — modern replacement for the deprecated `Iterator` key
- Each worker gets: `{ testId, url, requestCount, workerId, lambdaArn }`
- `lambdaArn` routes each worker to the correct regional Lambda (us-east-1 / ap-south-1 / eu-west-1)
...
cd frontend
npm install recharts framer-motion lucide-react @heroicons/react clsx tailwind-merge
```
...
| DYNAMODB_REGION     | Worker Lambdas (all 3) | `us-east-1` — forces cross-region writes      |
| STATE_MACHINE_ARN   | startTest              | ARN of your Step Functions state machine      |
| WORKER_ARN_US       | startTest              | ARN of worker Lambda in us-east-1             |
| WORKER_ARN_AP       | startTest              | ARN of worker Lambda in ap-south-1            |
| WORKER_ARN_EU       | startTest              | ARN of worker Lambda in eu-west-1             |
| VITE_API_URL        | React (frontend/.env)  | Your API Gateway base URL (no trailing slash) |
